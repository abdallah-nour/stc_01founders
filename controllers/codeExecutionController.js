...
const executeCode = async (req, res, next) => {
  ...
  const wrappedCode = getUserCodeWrapper({ challengeTitle, lang, userCode: code, params: challengeTest.params });
  const [err2, executionData] = await doAsync(executeCodeByAPI)(lang, wrappedCode);
...
const executeCodeCallback = async (req, res, next) => {
  ...
  if (userFunctionReturnValue === challengesTests[submission.challengeTitle].expectedResult)
  ...
  const isChallengeDoneBefore = user.solvedChallenges.find(title => title === submission.challengeTitle);
  if (!isChallengeDoneBefore) {
    user.solvedChallenges.push(submission.challengeTitle);
    const [err5, challenge] = await doAsync(() => Challenge.findOne({ title: submission.challengeTitle }))();
...
const getCodeStatus = async (req, res, next) => {
  ...
  const isDone = user.solvedChallenges.find(challengeTitle => challengeTitle === title);
